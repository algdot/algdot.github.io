...
1. 使用域名区分：不同环境的域名不一样。nginx按照域名解析到对应的目录
	1. 稳定环境：https://dev.domain.com/admin/index.html
	2. 分支环境：https://dev1.domain.com/admin/index.html
2. 后端接口：通过域名正则解析当前环境，所有请求带上header环境参数
### 方案二：路径区分
1. 使用路径区分：不同域名用请求路径区分。
	1.  稳定环境：https://dev.huajing.tech/ofa/admin/index.html
	2. 分支环境：https://dev1.huajing.tech/ofa/admin/dev1/index.html
2. 后端接口：同上
### 方案三：代理
